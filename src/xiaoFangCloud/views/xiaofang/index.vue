...
<div class="index flex">
    ...
    <div class="aside box-bg right-side">
        <el-scrollbar>
            <transition-group tag="ul" name="list" class="list">
                <li :class="{alarm:item.isAlarm}" v-for="(item,i) in projectList" :key="item.ProjectID" @click="selectProject(item,i)">
                    <div class="clearfix">
    ...
    <el-dialog append-to-body title="应急救援" top="20px" :visible.sync="show3" width="1532px" class="zw-dialog table-dialog">
        <div class="monitor-alarm-dialog">
            <Table :showOperation='false' :showOperationColumn='false' :tableLabel="tableLabel1" :getData="queryPageOrders"></Table>
        </div>
    </el-dialog>
...
import gauge from '../shuziYy/gauge.vue'
import dataItem from '../shuziYy/dataItem.vue'
import Table from '../manage/layout/table.vue'
import {
    Radar
} from '../shuziYy/radar.js'
import '../shuziYy/radar.css'
import '../shuziYy/index.scss'
import china from '@/mapJson/allCity.json'
import {HomePage} from '@/shuziYy/request/api.js'
    ...
    methods: {
        ...
        formatSosCount() {
            let data = this.rescueOrderCount[this.activeIndex] || [],
                columns = data.map(item => item.DeviceTypeName),
        ...
        queryData() {
            this.$socket({
                FAction: 'QueryOneFormAllProject',
        ...
        selectProject(item){
            ...
            this.$router.push(`/indexItem/${this.$route.params.formID}`)
        },
        /**
         * 获取实时告警数据
         */
